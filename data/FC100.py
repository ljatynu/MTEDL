...
class FC100(data.Dataset):
    def __init__(self, phase='train', do_not_use_random_transf=False):
        ...
        if self.phase=='train':
            ...
            # 将标签从大到小映射为min(label)到max(label)
            self.labels = self._remap_labels(self.labels)
    ...
    def _remap_labels(self, labels):
        ...
        sorted_unique_labels = np.sort(unique_labels)  # 从小到大排序

        # 创建一个从旧标签到新标签的映射
        label_map = {old_label: i for i, old_label in enumerate(sorted_unique_labels)}
